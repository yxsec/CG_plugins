...
registerPlugin('audio.stage-summary', async ({ intent, userId }: PluginContext): Promise<PluginResponse> => {
  try {
    ensureUser(userId)
    if(intent?.operation !== 'stage'){ return { status_code: SC.BAD_REQUEST, message: 'unsupported operation', data: {} } }
...
function buildPrompt({ previous2, previous, current, language, keywords }: StagePayload){
  const sections = [
    { title: '当前片段原文', body: current.trim() }
  ]
  ...
  const keywordText = keywords.length ? `偏好术语：${keywords.map((word) => word.replace(/[\r\n]+/g, ' ').trim()).filter(Boolean).join('，')}` : ''
  const instruction = [
    `请使用中文输出阶段性总结，保持专业口吻。`,
    '输出一个 JSON，包含：',
    '- summary: 一句话总结当前阶段。',
